...
// Files
ifstream input("input-c.txt");                   // This is the input file.
ofstream output("LogFile-cs20btech11055-C.txt"); // This is the output file.
// Waiting times
double WaitingTime = 0;
double MaxWaitingTime = 0;
// When a student comes, we call this function.
void Student(int k, int m, int n, int i, int mu)
{
    time_t now;
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        v.emplace_back(Student, k, m, n, i + 1, mu);
        double t = d(generator);
...
// Here, we can see that the program works for every k and it even allows multiple readers reading the same subject into the hall but the max number of readers allowed is m and here, we can see that because we are stopping the process after n students have entered, n students will definitely complete the execution and it doesn't lead to starvation.